...
## 2025-09-22 Tue
> **Process**: 
텍스트 추출(**Loading**) → 정제(**Cleaning**) → 메타데이터 결합 → 텍스트 분할(**Chunking**)


이번 프로젝트에 개발할 핵심 코드: src/ 폴더
#### src/loaders/:
...
### ⚪ 데이터 추출(Loading & Extraction)

**타겟 데이터**: 골든셋(golden set)을 지정하는 것이 아닌 **전체 데이터**에 대해서 전수 조사 하는 방식으로 진행
1)텍스트, 2)이미지 3)테이블

- HWP마다 텍스트, 표, 이미지 구성 비율이 다름 → 추출 파이프라인 설계 기준이 됨.
- 어떤 건 본문 위주, 어떤 건 표 위주(예산, 일정), 어떤 건 이미지 스캔(스캔본 PDF)일 수 있음.
- 자동 추출 정확도를 평가하려면, **ground truth에 가까운 데이터 맵**이 필요.



#### 1. 텍스트 추출
- 데이터 포맷: 
  1) hwp 원본 직접 추출: `pyhwp` / `hwp5txt`
      - 구조적으로 1개의 데이터가 추출이 불가능
      - pyhwp 라이브러리가 내부 텍스트를 제대로 읽어오지 못하는 특수한 구조
  2) hwp -> HTML
      - HWP의 문서 구조(제목, 문단 등)를 매우 잘 보존
      - 표(Table)'의 행/열 구조를 <table> 태그로 그대로 살려줘서 파이썬(BeautifulSoup)으로 파싱하기 좋음  

    3) hwp -> pdf
      - PDF 생태계는 너무 커서 도구가 많음 (`pdfplumber`, `PyMuPDF`, `Camelot`, `pdf2docx` 등)
      - 표 추출은 Camelot/Tabula, 이미지 OCR은 Tesseract + LayoutParser.
      - 실험 환경에 독립적 (윈도우/리눅스 모두 가능).
  
  4) hwp -> hwpx
    - 
  
  5) hwp -> docx
      - docx는 사실상 XML 기반이라 표 구조(w:tbl, w:tr, w:tc) 그대로 파싱 가능.
      - Python에서 python-docx 같은 라이브러리로 표/문단 쉽게 접근 가능.
      - 이미지도 docx 안에 media 폴더로 저장되니까 OCR 파이프라인 연결 가능.
      - 변환 fidelity 이슈


##### 데이터 인벤토리 예시

| id (파일명)     | 텍스트 길이(페이지/줄수) | 표 개수 | 표 주요항목   | 이미지 개수 | 이미지 설명  | 특이사항     |
| ------------ | -------------- | ---- | -------- | ------ | ------- | -------- |
| RFP\_001.hwp | 약 10p          | 2    | 예산, 일정   | 1      | 조직도     | 본문 글자 깨짐 |
| RFP\_002.hwp | 약 25p          | 4    | 예산, 제출방식 | 0      | 없음      | 표 병합 많음  |
| RFP\_003.hwp | 약 8p           | 1    | 예산       | 3      | 도면, 스캔본 | OCR 필요   |
